...
- 挑窗口时先**只看当前最前 app 自己的窗口**，该 app 没有可用窗口时才退回全屏扫描（桌面、菜单栏 app、或者由后台 agent 弹出、不会激活自己的对话框）。窗口层级按用途分三档：
  - **layer 0**：普通窗口，任何 app 都算。
  - **layer 8 及以上**：系统对话框带 —— 模态面板、认证弹窗、宗卷选择框。它们在最前面是因为有人特意把它们摆在最前面，所以不论属于谁都能当焦点。（这正是「连接服务器」那次的坑：小的「正在连接 smb://…」浮条能当焦点，它弹出的宗卷选择框是模态面板却被层级上限挡掉，于是洞一直停在小条上。）
  - **layer 1–7**：置顶装饰带 —— 浮动 HUD、桌面宠物、歌词条、贴边浮层。只认最前 app 自己的窗口（Quick Look 预览、浮动面板正落在这里），免得别家 app 的宠物抢走焦点。
  - 上限是 Dock（20）：Dock、菜单栏（24）、弹出菜单（101）等永远不能成为焦点。
